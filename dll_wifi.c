...
CnetTimerID lasttimer2 = NULLTIMER;
struct dll_wifi_state *tempstate;
struct wifi_frame tempframe;
size_t tempframe_length;
//int collisions = 1;
                    ...
                    uint16_t length)
{
  ...
  if(CNET_carrier_sense(state->link) == 1) {
    return; 
    //tempframe_length = WIFI_HEADER_LENGTH + length;
    //fprintf(stdout, "wifi line busy, framesize is %d\n", (int)tempframe_length);
  }
  else {
    ...
    //fprintf(stdout, "wifi success, framesize is %d\n", (int)frame_length);
  }
}
